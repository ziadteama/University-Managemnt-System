    ...
    public List<CanEnroll> getCanEnroll(int userId) {
        ...
        String query = """
            SELECT
                ...
                GROUP_CONCAT(DISTINCT CASE WHEN s.class_type = 'lecture' THEN CONCAT(s.period, ' at ', s.location) END) AS lecture_time,
                GROUP_CONCAT(DISTINCT CASE WHEN s.class_type = 'tutorial' THEN CONCAT(s.period, ' at ', s.location) END) AS tutorial_time
            FROM 
                schedules s
            JOIN 
